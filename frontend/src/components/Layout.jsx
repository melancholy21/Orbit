...
const Layout = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen relative overflow-hidden">
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[100] focus:bg-background focus:text-foreground focus:p-4 focus:border focus:border-border focus:rounded-md focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-primary"
      >
        Skip to content
      </a>
      {/* Top App Bar */}
      ...
      <header className="fixed top-0 left-0 right-0 h-14 border-b border-border bg-background z-50 flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="text-muted-foreground hover:text-[#1DB954] hover:bg-muted/40 transition-colors w-11 h-11 flex items-center justify-center rounded-full"
            aria-label="Spotify Lobby"
          >
          ...
          <button 
            className="text-muted-foreground hover:text-foreground hover:bg-muted/40 transition-colors relative w-11 h-11 flex items-center justify-center rounded-full"
            onClick={() => navigate('/notifications')}
            aria-label="Notifications"
          >
            <Bell size={20} />
            {unreadCount > 0 && (
              <div className="absolute top-1 right-1 w-[18px] h-[18px] bg-destructive rounded-full flex items-center justify-center border border-background">
          ...
          <button 
            ...
            className="text-muted-foreground hover:text-foreground hover:bg-muted/40 transition-colors w-11 h-11 flex items-center justify-center rounded-full"
            aria-label="Settings"
          >
            <Settings size={20} />
          </button>
      ...
      {(() => {
        ...
        return (
          <main id="main-content" tabIndex="-1" className={`outline-none flex-grow flex justify-center relative z-10 ${isChatView ? '' : `pt-20 ${isMiniPlayerVisible ? 'pb-36' : 'pb-20'} p-4`}`}>
